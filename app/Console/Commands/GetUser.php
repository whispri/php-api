...
{
    protected $signature = 'kafka:getuser {--reset-offset : Reset offset to earliest}';
    protected $description = 'Consume Kafka messages';


    protected KafkaProducerService $kafka;

    public function __construct(KafkaProducerService $kafka)
    {
        parent::__construct();
        $this->kafka = $kafka;
    }
    ...
    public function handle()
    {
        ...
        $consumer = KafkaConsumerBuilder::create()
            ...
            ->withAdditionalSubscription('get-user')
            ->build();
    ...
    private function processMessage(KafkaConsumerMessageInterface $message): void
    {
        try {
            ...
            if (json_last_error() === JSON_ERROR_NONE) {
                ...
                \Log::info($user);
                $this->kafka->sendMessage('response-user',  ['user' => $user]);
